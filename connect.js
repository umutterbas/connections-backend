...
const INDEX = fs.readFileSync(path.resolve(__dirname, "client", "index.html"), {
  encoding: "utf8",
});
...
async function main() {
  ...
  app.use((req, res, next) => {
    ...
    if (req.url === "/twitter/authenticate")
      return twitter("authenticate")(req, res);
    if (req.url === "/twitter/authorize") return twitter("authorize")(req, res);
    if (req.url.startsWith("/twitter/callback"))
  ...
  function handleTwitterCallback(req, res) {
    ...
    const { oauth_verifier: oauthVerifier } =
      querystring.parse(req.url.substring("/twitter/callback?".length)) || {};
    ...
    return getOAuthAccessTokenWith({
      oauthRequestToken,
      oauthRequestTokenSecret,
      oauthVerifier,
      ...
      req.session.save(() => {
        ...
        res.setHeader("Set-Cookie", [
          `twitter_screen_name=${user.screen_name}; Path=/;`,
        ]);
        res.end();
      });
    });
